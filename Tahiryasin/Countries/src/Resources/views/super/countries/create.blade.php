...
@section('content')
    <div class="content">
        ...
        <form method="POST" action="{{ route('super.countries.store') }}" @submit.prevent="onSubmit" enctype="multipart/form-data">
                    ...
                    <accordian :title="'{{ __('countries::app.countries.general') }}'" :active="true">
                        <div slot="body">
                            ...
                            <div class="control-group" :class="[errors.has('code') ? 'has-error' : '']">
                                ...
                                <input v-validate="'required|min:2|max:2'" class="control" id="code" name="code" value="{{ old('code') }}" data-vv-as="&quot;{{ __('countries::app.countries.code') }}&quot;" style="text-transform:uppercase" v-code/>
                                <span class="control-error" v-if="errors.has('code')">@{{ errors.first('code') }}</span>
                            </div>

                            <div class="control-group" :class="[errors.has('name') ? 'has-error' : '']">
                                <label for="name" class="required">{{ __('countries::app.countries.name') }}</label>
                                <input v-validate="'required'" class="control" id="name" name="name" data-vv-as="&quot;{{ __('countries::app.countries.name') }}&quot;" value="{{ old('name') }}"/>
